...
class Octree:
  ...
  def get_inv_neigh(
      self, depth: int, kernel: str = "333", stride: int = 1, nempty: bool = False
  ):
        assert (
            stride == 1
        ), "Implicit gemm only supports stride = 1, got stride = {}".format(stride)
        ...
        if kernel == "333":
            ...
            return inv_neigh[:, self.lut_kernel[kernel]]
        else:
            raise ValueError("Unsupported kernel {}".format(kernel))
