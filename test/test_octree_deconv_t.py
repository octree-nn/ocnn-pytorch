...
def test_deconv_stride1(depth, out_ratio, kernel_size):
  atol = 5e-3
  nempty = False
  dtype = torch.float32
  ...
  with torch.no_grad():
    conv_triton.weights.copy_(conv_im2col.weights.permute(1, 0, 2))
  ...
  assert torch.allclose(
    conv_im2col.weights.grad.permute(1, 0, 2),
    conv_triton.weights.grad,
    atol=atol,
  ), "got weight gradient error rate {}".format(
    calc_err(
      conv_im2col.weights.grad, conv_triton.weights.grad.permute(1, 0, 2)
    )
  )
  assert torch.allclose(
